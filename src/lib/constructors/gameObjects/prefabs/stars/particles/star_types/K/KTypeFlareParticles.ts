...
export class KTypeFlareParticles {
    ...
    constructor(scene: BABYLON.Scene, mesh: BABYLON.Mesh, diameter: number) {
        ...
        if(diameter > 2) {
            this.type = "giant"
        this.particleSystem.minScaleX = getRandomNumberBetween(0.75, 2.5);
        this.particleSystem.minScaleY = getRandomNumberBetween(0.75, 2.5);
        this.particleSystem.maxScaleX = getRandomNumberBetween(0.75, 2.5);
        this.particleSystem.maxScaleY = getRandomNumberBetween(0.75, 2.5);
        }
        else {
            this.type = "dwarf"
